...
**Native module rebuild:** `better-sqlite3` is a native Node module that must be compiled against Electron's Node ABI, not the system Node. The `postinstall` hook runs `tools/rebuild-native.js` which calls `node-gyp` directly with `--target=<electron-version> --dist-url=https://electronjs.org/headers`. Don't use `npx electron-rebuild` or `@electron/rebuild` directly — they silently fall through to `prebuild-install`, which downloads a prebuilt targeting the system Node ABI (wrong) and reports "Rebuild Complete" anyway. Symptom of the wrong ABI: `npm start` crashes with `NODE_MODULE_VERSION 127 vs 143`. `npm test` automatically rebuilds for system Node (via `pretest`) and back for Electron (via `posttest`).

**`npm start` is self-healing.** The `prestart` hook (`tools/check-electron-abi.js`) checks the `.built-for` marker file written by `rebuild-native.js`. If the marker is missing or doesn't match the current Electron version, prestart triggers a rebuild before launching. So **you should not need to run `node tools/rebuild-native.js` manually** — `npm start` handles it. The pretest hook (`tools/invalidate-native-marker.js`) deletes the marker before rebuilding for system Node, so the next `npm start` correctly detects the mismatch and rebuilds for Electron. If you're running tests directly via `npx vitest run` (bypassing the npm test lifecycle), the rebuild hooks don't fire and the binary stays in whatever state it was in.
